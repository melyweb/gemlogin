...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Script parameters are stored in the script itself, no need to process them here
    ...
    if (empty($errors)) {
        ...
        try {
            // Insert schedule
            $stmt = $db->prepare("INSERT INTO schedules (name, script_id, start_time, end_time, profile_delay, loop_delay, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, NOW())");
            $user_id = $_SESSION['user_id'];
            $stmt->bind_param("ssssiis", $name, $script_id, $start_time, $end_time, $profile_delay, $loop_delay, $user_id);
            ...
            $stmt = $db->prepare("INSERT INTO schedule_profiles (schedule_id, profile_id) VALUES (?, ?)");
            $stmt->bind_param("ii", $schedule_id, $profile_id);
            
            foreach ($profile_ids as $profile_id) {
                if (!$stmt->execute()) {
                    throw new Exception("Failed to associate profile {$profile_id}: " . $stmt->error);
                }
            }
            
            $db->commit();
            
            set_flash_message('Schedule created successfully.', 'success');
...
// Default values for the form (including auto-generating default times using Vietnam timezone)
$vietnamTz = new DateTimeZone('Asia/Saigon'); // UTC+7
$now = new DateTimeImmutable('now', $vietnamTz);
$fiveMinLater = $now->modify('+5 minutes');
$default_start_time = $fiveMinLater->format('Y-m-d H:i'); // 5 minutes from now

// Calculate end time (24 hours from now)
$oneDayLater = $now->modify('+24 hours');
$default_end_time = $oneDayLater->format('Y-m-d H:i'); 
$default_profile_delay = DEFAULT_PROFILE_DELAY;
$default_loop_delay = DEFAULT_LOOP_DELAY;

// Pre-selected script ID from query string (if any)
$selected_script_id = isset($_GET['script_id']) ? clean_input($_GET['script_id']) : '';
...
$script_parameters = [];
if (!empty($selected_script_id)) {
    $stmt = $db->prepare("SELECT parameters FROM scripts WHERE id = ?");
    $stmt->bind_param("s", $selected_script_id);
    $stmt->execute();
    $result = $stmt->get_result();
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="script_id" name="script_id" required>
                            ...
                            <?php foreach ($scripts as $script): ?>
                                <option value="<?php echo $script['id']; ?>" <?php echo ($selected_script_id === $script['id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control datetimepicker" id="start_time" name="start_time" value="<?php echo htmlspecialchars($start_time ?? $default_start_time); ?>" required>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control datetimepicker" id="end_time" name="end_time" value="<?php echo htmlspecialchars($end_time ?? $default_end_time); ?>" required>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="profile_delay" name="profile_delay" value="<?php echo (int)($profile_delay ?? $default_profile_delay); ?>" min="0" required>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="loop_delay" name="loop_delay" value="<?php echo (int)($loop_delay ?? $default_loop_delay); ?>" min="0" required>
                                        ...
                                        <?php foreach ($group['profiles'] as $profile): ?>
                                            <div class="list-group-item">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="profile_ids[]" value="<?php echo $profile['id']; ?>" id="profile_<?php echo $profile['id']; ?>">
